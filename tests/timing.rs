...
fn time_pipeline_steps() {
    ...
    println!("fixture size: {} bytes ({:.1} KB)", raw.len(), raw.len() as f64 / 1024.0);
    ...
    println!("EXIF extract:  {:>8.2} ms", t.elapsed().as_secs_f64() * 1000.0);
    ...
    println!("output JPEG:   {} bytes ({:.1} KB)", jpeg.len(), jpeg.len() as f64 / 1024.0);
}